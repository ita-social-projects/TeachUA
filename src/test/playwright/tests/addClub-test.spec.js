...
import {
    CLUB_CATEGORIES,
    NEW_LOCATION_CORRECT_DETAILS,
    NEW_CLUB_CORRECT_CONTACT_DETAILS,
    NEW_CLUB_INCORRECT_CONTACT_DETAILS,
    NEW_CLUB_CORRECT_DETAILS,
    NEW_CLUB_INCORRECT_DETAILS,
    NEW_LOCATION_INCORRECT_DETAILS,
} from "../constants/clubInformation.constants";
import {
    ADD_CLUB_VALIDATION_ERROR_MESSAGE,
    ADD_LOCATION_VALIDATION_ERROR_MESSAGE,
    SUCCESS_CLUB_CREATION_MESSAGE,
} from "../constants/messages.constants";
import { CREATE_CLUB_REQUEST } from "../constants/api.constants"

let page, apiService, addClubPage, homePage, userPage, clubInfoPage;
...
test("Verify that a new club can be succesfully added", async () => {
    ...
    await addClubPage.fillInputField(
        addClubPage.locationCoordinatesField,
        NEW_LOCATION_CORRECT_DETAILS.locationCoordinates
    );
    ...
    await addClubPage.verifyElementVisibilityAndText(
        addClubPage.clubCreatedSuccessMessage,
        true,
        SUCCESS_CLUB_CREATION_MESSAGE
    );
    ...
    await clubInfoPage.expectElementToContainText(
        ...
        NEW_CLUB_CORRECT_CONTACT_DETAILS.email
    );
    await clubInfoPage.expectElementToContainText(
        clubInfoPage.clubPageContactData,
        NEW_CLUB_CORRECT_CONTACT_DETAILS.facebook
    );
    await clubInfoPage.expectElementToContainText(
        clubInfoPage.clubPageContactData,
        NEW_CLUB_CORRECT_CONTACT_DETAILS.phoneNumber
    );
    await clubInfoPage.expectElementToContainText(
        clubInfoPage.clubPageContactData,
        NEW_CLUB_CORRECT_CONTACT_DETAILS.whatsup
    );
    await clubInfoPage.expectElementToContainText(
        clubInfoPage.clubPageContactData,
        NEW_CLUB_CORRECT_CONTACT_DETAILS.webPage
    );
    await clubInfoPage.verifyElementVisibilityAndText(
...
test("Verify validation error messages appearence/disappearence on the first step", async () => {
    await addClubPage.proceedToNextStep();
    await addClubPage.verifyElementVisibilityAndText(
        addClubPage.nameFieldErrorMessage,
        true,
        ADD_CLUB_VALIDATION_ERROR_MESSAGE.nameIsMandatory
    );
    await addClubPage.verifyElementVisibilityAndText(
        addClubPage.categoryFieldErrorMessage,
        true,
        ADD_CLUB_VALIDATION_ERROR_MESSAGE.categoryIsMandatory
    );
    await addClubPage.verifyElementVisibilityAndText(
        addClubPage.ageFromFieldErrorMessage,
        true,
        ADD_CLUB_VALIDATION_ERROR_MESSAGE.ageIsMandatory
    );
    await addClubPage.verifyElementVisibilityAndText(
        addClubPage.ageToFieldErrorMessage,
        true,
        ADD_CLUB_VALIDATION_ERROR_MESSAGE.ageIsMandatory
    );
    ...
    await addClubPage.verifyElementVisibilityAndText(
        addClubPage.nameFieldErrorMessage,
        true,
        ADD_CLUB_VALIDATION_ERROR_MESSAGE.nameIsIncorrect
    );
    await addClubPage.fillInputField(addClubPage.clubNameField, NEW_CLUB_CORRECT_DETAILS.clubTitle);
...
test("Verify validation error messages appearence/disappearence on the second step", async ({ page }) => {
    ...
    await addClubPage.verifyElementVisibilityAndText(
        addClubPage.locationNameFieldErrorMessage,
        true,
        ADD_LOCATION_VALIDATION_ERROR_MESSAGE.locationNameIsMandatory
    );
    await addClubPage.verifyElementVisibilityAndText(
        addClubPage.locationCityErrorMessage,
        true,
        ADD_LOCATION_VALIDATION_ERROR_MESSAGE.cityIsMandatory
    );
    ...
    await addClubPage.verifyElementVisibilityAndText(
        addClubPage.locationCoordinatesFieldErrorMessage,
        true,
        ADD_LOCATION_VALIDATION_ERROR_MESSAGE.coordinatesAreIncorrect
    );
    await addClubPage.verifyElementVisibilityAndText(
        addClubPage.locationPhoneFieldErrorMessage,
        true,
        ADD_LOCATION_VALIDATION_ERROR_MESSAGE.phoneIsMandatory
    );
    ...
    await addClubPage.verifyElementVisibilityAndText(
        addClubPage.locationNameFieldErrorMessage,
        true,
        ADD_LOCATION_VALIDATION_ERROR_MESSAGE.locationNameIsIncorrect
    );

    await addClubPage.fillInputField(addClubPage.locationAddressField, NEW_LOCATION_INCORRECT_DETAILS.locationAddress);
    await addClubPage.verifyElementVisibilityAndText(
        addClubPage.locationAddressFieldErrorMessage,
        true,
        ADD_LOCATION_VALIDATION_ERROR_MESSAGE.addressIsIncorrect
    );

    await addClubPage.fillInputField(
        addClubPage.locationCoordinatesField,
        NEW_LOCATION_INCORRECT_DETAILS.locationCoordinates
    );
    await addClubPage.verifyElementVisibilityAndText(
        addClubPage.locationCoordinatesFieldErrorMessage,
        true,
    ...
    await addClubPage.fillInputField(
        addClubPage.locationCoordinatesField,
        NEW_LOCATION_INCORRECT_DETAILS.locationCoordinatesWithLetters
    );
    await addClubPage.verifyElementVisibilityAndText(
        addClubPage.locationCoordinatesFieldErrorMessage,
        true,
        ADD_LOCATION_VALIDATION_ERROR_MESSAGE.coordinatesCantHaveLetters
    );

    await addClubPage.fillInputField(
        addClubPage.locationPhoneNumberField,
        NEW_LOCATION_INCORRECT_DETAILS.locationPhone
    );
    await addClubPage.verifyElementVisibilityAndText(
        addClubPage.locationPhoneFieldErrorMessage,
        true,
        ADD_LOCATION_VALIDATION_ERROR_MESSAGE.phoneIsIncorrect
    );
    ...
    await addClubPage.proceedToNextStep();
    await addClubPage.verifyElementVisibilityAndText(
        addClubPage.clubPhoneFieldErrorMessage,
        true,
        ADD_CLUB_VALIDATION_ERROR_MESSAGE.phoneIsMandatory
    );

    await addClubPage.fillInputField(
        addClubPage.clubPhoneNumberField,
        NEW_CLUB_INCORRECT_CONTACT_DETAILS.phoneNumberIncorrectFormat
    );
    await addClubPage.verifyElementVisibilityAndText(
        addClubPage.clubPhoneFieldErrorMessage,
        true,
        ADD_CLUB_VALIDATION_ERROR_MESSAGE.phoneIsIncorrect
    );
    await addClubPage.fillInputField(
        addClubPage.clubPhoneNumberField,
        NEW_CLUB_INCORRECT_CONTACT_DETAILS.phoneNumberIncorrectFormatWithLetters
    );
    await addClubPage.verifyElementVisibilityAndText(
        addClubPage.clubPhoneFieldErrorMessage,
        true,
        ADD_CLUB_VALIDATION_ERROR_MESSAGE.phoneIsIncorrectCantHaveLetters
    );
    await addClubPage.fillInputField(addClubPage.clubPhoneNumberField, NEW_CLUB_CORRECT_CONTACT_DETAILS.phoneNumber);
    ...
    await addClubPage.verifyElementVisibilityAndText(
        addClubPage.clubEmailFieldErrorMessage,
        true,
        ADD_CLUB_VALIDATION_ERROR_MESSAGE.emailIsIncorrect
    );
    await addClubPage.fillInputField(addClubPage.clubEmailField, NEW_CLUB_CORRECT_CONTACT_DETAILS.email);
...
test("Verify validation error messages appearence/disappearence on the third step", async ({ page }) => {
    ...
    await addClubPage.verifyElementVisibilityAndText(
        addClubPage.clubDescriptionErrorMessage,
        true,
        ADD_CLUB_VALIDATION_ERROR_MESSAGE.descriptionIsIncorrect
    );
    await addClubPage.fillInputField(addClubPage.descriptionTextArea, NEW_CLUB_INCORRECT_DETAILS.clubDescription);
    await addClubPage.verifyElementVisibilityAndText(
        addClubPage.clubDescriptionErrorMessage,
        true,
        ADD_CLUB_VALIDATION_ERROR_MESSAGE.descriptionIsIncorrectCharLimit
    );
    await addClubPage.fillInputField(addClubPage.descriptionTextArea, NEW_CLUB_CORRECT_DETAILS.clubDescription);
    await addClubPage.verifyElementVisibilityAndText(addClubPage.clubDescriptionErrorMessage, false);
});

test("Verify that tooltips and warning messages appear", async () => {
    // Remove the club with the test club_title if it exists
    await apiService.createNewClub(CREATE_CLUB_REQUEST);
    await addClubPage.fillInputField(addClubPage.clubNameField, CREATE_CLUB_REQUEST.body.name);
    await addClubPage.toggleStatedCheckboxes(CLUB_CATEGORIES.acting);
    await addClubPage.setAcceptableChildAge(
        NEW_CLUB_CORRECT_DETAILS.clubEnterAge,
        NEW_CLUB_CORRECT_DETAILS.clubClosingAge
